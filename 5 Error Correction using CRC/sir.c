...
{
    for(e=0;e<N;e++)
        cs[e]=t[e];               //copy the padded data to checksum array
    
    //before cmmg here e = N
    do{
...
int main()
{
    ...
    a=strlen(t);
    for(e=a;e<a+N-1;e++)    //data is padded with generator polynomial - 1 length (0's)
        t[e]='0';
    ...
    if(e==0)
    {
        do{
            ...
            scanf("%d",&e);
        }while(e==0 || e>a+N-1); //if position not in limit, ask again

        t[e-1]=(t[e-1]=='0')?'1':'0';   //convert to index and change bit
        printf("\n----------------------------------------");
        printf("\nErroneous data : %s\n",t);
    }
    crc();
